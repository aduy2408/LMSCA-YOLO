...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        tal_topk = int(self.yaml.get("tal_topk", 10))
        tal_topk2 = self.yaml.get("tal_topk2")
        tal_topk2 = int(tal_topk2) if tal_topk2 is not None else None
        return E2ELoss(self) if getattr(self, "end2end", False) else v8DetectionLoss(self, tal_topk, tal_topk2)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in {EnSimAM, LearnableEnSimAM}:
            c2 = ch[f]
        elif m in {P4CrossScaleLocalAttention, SAGRI, SAGRIConvDown, P2PartialKVToP3, P5LocalToP4}:
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
